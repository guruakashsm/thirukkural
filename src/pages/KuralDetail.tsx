...
export default function KuralDetail() {
  ...
  const { t, lang, getMeaning, getChapterName, getCategoryName } = useLanguage()
  ...
  const category = categories.find(c => c.chapters.some(ch => ch.number === kural.chapter))
  const chapter = category?.chapters.find(ch => ch.number === kural.chapter)
  const categoryLabel = category
    ? getCategoryName(category.englishName) || t(category.englishName.toLowerCase()) || category.name
    : ''
  const breadcrumbChapterName = lang === 'ta' ? kural.chapterName : (chapterNameTranslated || kural.chapterEnglish)
  const chapterLabel = `${kural.chapter}. ${breadcrumbChapterName}`
  const kuralLabel = `${t('kural')} #${kural.number}`
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 animate-fade-in">
      <Breadcrumbs
        items={[
          { label: t('home'), to: '/' },
          ...(category ? [{ label: categoryLabel, to: `/category/${encodeURIComponent(category.name)}` }] : []),
          { label: chapterLabel, to: `/chapter/${kural.chapter}` },
          { label: kuralLabel },
        ]}
      />
